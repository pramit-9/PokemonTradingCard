...
module.exports = {
    index: async (req, res) => {
        try{            
            const cardsQuery = 'SELECT pokemons.*, categories.name AS category_name FROM pokemons JOIN categories ON pokemons.category_id = categories.id';
            const cards = await query(cardsQuery);
            // console.log("cards", cards[0].category_name);
